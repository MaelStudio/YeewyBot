...
module.exports = {
	...
	async execute(message, args) {
        ...
        const target = message.mentions.members.first() || message.guild.members.cache.find(m => m.user.username == args[0] || m.user.tag == args[0] || m.id == args[0]);
        ...
        if(!message.guild.bans.get(target.id)) {
			let embed = new MessageEmbed()
				.setColor('#ff3a3a')
				.setTitle('⚠️ • Error')
